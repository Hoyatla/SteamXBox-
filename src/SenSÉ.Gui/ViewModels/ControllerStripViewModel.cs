...
public partial class ControllerStripViewModel : ObservableObject
{
    ...
    public void Refresh()
    {
        ControllerIdentityFactory.DurableKeyResolver ??= path => SenSÉ.Windows.DeviceTree.DurableKeyFor(path);
        ...
        var roster = ControllerRoster.Build(
            ...
            // tell which is which.
            //
            // The user sees the slot of the controller they are holding. The pad SenSÉ creates
            ...
            // attached.
            SenSÉ.Windows.XInputControllerSource.ConnectedSlots()
                .Where(slot => !SenSÉ.Windows.XInputDurableIdentity.IsEmulatedSlot(slot)),

            SafeDualSensePaths(),
            slot => SenSÉ.Windows.XInputDurableIdentity.For(slot));
    ...
    private static IEnumerable<string> SafeHidPaths()
    {
        try
        {
            return new SenSÉ.Hid.SteamHidDiscovery()
                .ListValveDevices()
    ...
    private static IEnumerable<string> SafeDualSensePaths()
    {
        try
        {
            return SenSÉ.Hid.DualSenseControllerSource.Discover()
                .Select(d => d.DevicePath)
                .ToList();
        }
        catch (Exception ex)
    ...
    public void Rename(ControllerSlotItem item)
    {
        ...
        var dialog = new SenSÉ.Gui.Views.RenameDialog(current)
        {
