...
public sealed class CoreProcessService : IDisposable
{
    ...
    public bool AdoptRunningInstance()
    {
        lock (_lock)
        {
            ...
            try
            {
                ...
                var found = Process.GetProcessesByName("SenSÉ.Core")
    ...
    public string GetCorePath()
    {
        ...
        var corePath = Path.Combine(dir, "SenSÉ.Core.exe");
        if (File.Exists(corePath))
            return corePath;

        var alt = Path.Combine(dir, "..", "SenSÉ.Core.exe");
    ...
    public bool Start(ProfileData profile)
    {
        lock (_lock)
        {
            ...
            try
            {
                ...
                SenSÉ.Windows.ChildProcesses.Adopt(
