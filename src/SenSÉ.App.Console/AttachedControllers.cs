...
public static class AttachedControllers
{
    ...
    /// The XInput slots that were occupied before SenSÉ created anything virtual. Required, not
    /// optional.
    /// </param>
    /// <remarks>
    /// <para>
    /// <b>The virtual pads SenSÉ creates are themselves XInput devices.</b> Reading every slot
    ...
    /// place that knows what SenSÉ is actually holding — which is the only safe answer to "what
    ...
    /// staying hidden from every other application until SenSÉ closes. A pad put to sleep at
        ...
        Action<string>? log = null)
    {
        ...
        if (wantsXInput)
        {
            ...
            {
                // Physical pads only — the helper refuses a ViGEm one, which is what keeps SenSÉ
