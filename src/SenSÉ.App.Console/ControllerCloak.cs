...
/// <summary>
/// Hides the controllers SenSÉ is holding from every other application.
/// </summary>
/// <remarks>
/// Without this a pad reaches two places at once. SenSÉ reads it and acts on it — closing the
...
/// all and nothing on screen to connect that to SenSÉ. The note file is what bounds that: what
/// was hidden is written down before it is hidden, so the next launch can undo exactly that much.
/// </para>
///
/// <para>
/// <b>Only ever what SenSÉ hid.</b> The first version cleared HidHide entirely at startup, and
...
public static class ControllerCloak
{
    ...
    /// meant a machine with two copies of SenSÉ had two notes, and neither could repair the
    ...
    private const string TurnedCloakOn = "cloak-on-by-SenSÉ";
    ...
    public static void Apply(DiagnosticLog log)
    {
        ...
        if (!Available)
        {
            ...
            if (!_warned)
            {
                ...
                log.Info(
                    ...
                    + "a button SenSÉ uses also reaches whatever is in the foreground. Install it "
        ...
        try
        {
            ...
            // and SenSÉ hid nothing at all while believing it had.
    ...
    /// </summary>
    /// <remarks>
    /// Hiding a device is how SenSÉ stops a button reaching the foreground as well as itself.
    ...
    /// the session. A pad put to sleep in the evening was invisible to games until SenSÉ was
    ...
    public static void Release(DiagnosticLog log)
    {
        ...
        catch (Exception exception)
        {
            ...
            log.Info(
                ...
                + "SenSÉ is started again, or until HidHide-Off.cmd is run.");
        }
    }
}
