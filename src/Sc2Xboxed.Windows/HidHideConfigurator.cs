...
public sealed partial class HidHideConfigurator
{
    ...
    /// keyboards and mice share the vendor identifier of Xbox pads, and the virtual pad SteamXBox
    ...
    /// when every device SteamXBox holds is already on the list and cloaking is off, none of them is
    ...
    private string Run(IReadOnlyList<string> arguments)
    {
        ...
        // Bounded as well. A tool that never exits must not become a SteamXBox that never starts,
    ...
    /// were then written into SteamXBox's note as its own, and given back on the way out. The class
