...
namespace SenSÉ.Shell.Theming.Windows;

/// <summary>
/// Remembers what Windows looked like before SenSÉ changed it.
/// </summary>
/// <remarks>
/// This is the piece the whole Windows-theming plan rests on, and it is written first for that
/// reason. Anything that writes into the user's hive must be undoable, or uninstalling SenSÉ
...
public sealed class WindowsStateBackup
{
    ...
    public WindowsStateBackup(string? path = null)
        => _path = path ?? Path.Combine(
            ...
            "SenSÉ", "windows-state-backup.json");
