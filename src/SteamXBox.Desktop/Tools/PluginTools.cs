...
public static class PluginTools
{
        ...
        foreach (var manifest in scan.Loaded.Where(m => m.Kind == PluginCategory.Tool))
        {
            ...
            tools.Add(new ToolDescriptor(
                ...
                Compte: manifest.Does.Equals(PluginActions.Activite, StringComparison.OrdinalIgnoreCase)
                    ? SteamXBox.Desktop.Activite.ActiviteWindow.Compte
                    : null));
    ...
    private static void Rouille(PluginManifest manifeste, Action<string>? log)
    {
        ...
        try
        {
            foreach (var gel in SteamXBox.Tools.Generation.FluxGel.Examiner(manifeste, Resoudre))
            {
        ...
        try
        {
            switch (does.ToLowerInvariant())
            {
                ...
                case PluginActions.Generation:
                    return SteamXBox.Tools.Generation.ComfyServer.Ouvrir(log);

                case PluginActions.Flux:
                    return SteamXBox.Tools.Generation.FluxTravail.Lancer(target, log);

                case PluginActions.Sequence:
                    return SteamXBox.Tools.Generation.SequenceAnimee.Lancer(target, log);

                case PluginActions.Assistant:
                    return Assistant.AssistantWindow.Ouvrir(log);

                case PluginActions.Activite:
                    return SteamXBox.Desktop.Activite.ActiviteWindow.Ouvrir(log);

                case PluginActions.Modeles:
                    return SteamXBox.Desktop.Modeles.ModelesWindow.Ouvrir(log);
    ...
    private static string Python(string target, Action<string>? log)
    {
        ...
        var commande =
            $"title Outil SteamXBox & echo Demarrage, PyTorch met quelques secondes a se charger... & echo. "
    ...
    private static bool Repond(string executable)
    {
        try
        {
            ...
            var (sortie, erreur) = SteamXBox.Tools.Serveurs.Tuyaux.Vider(
    ...
    {
        // Le Python embarque du produit passe avant tout le reste : un dossier que SteamXBox porte
    ...
    /// à <c>SteamXBox.Tools</c>, où c'est atteignable par les tests. C'est la même répartition que
    ...
    private static string Image(string target, Action<string>? log)
    {
        ...
        return SteamXBox.Tools.Images.ImageUpscale.Agrandir(
    ...
    private static string Video(string target, Action<string>? log)
    {
        ...
        return SteamXBox.Tools.Video.VideoUpscale.Agrandir(
    ...
    private static string Convert(string target, Action<string>? log)
    {
        ...
        if (SteamXBox.Tools.Documents.PdfOcr.Handles(input, format))
        {
            return SteamXBox.Tools.Documents.PdfOcr.Convert(input, folder);
        }
        ...
        // lui appartient.
        if (SteamXBox.Tools.Documents.PdfPageToDocument.Handles(input, format))
        {
            var rendu = SteamXBox.Tools.Documents.PdfPageToDocument.Convert(input, folder);
