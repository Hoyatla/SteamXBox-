...
using SteamXBox.Desktop.Settings;
using SteamXBox.Desktop.Tools;
using SteamXBox.Tools.Activite;

namespace SteamXBox.Desktop.Activite;

/// <summary>
/// Ce qui tourne au nom de SteamXBox, et de quoi l'arrêter.
/// </summary>
/// <remarks>
...
public partial class ActiviteWindow : Window
{
    ...
    {
        var arretees = SteamXBox.Tools.Serveurs.Ressources.ArreterTout();
    ...
    {
        var (utilise, total) = SteamXBox.Tools.Serveurs.Ressources.MemoireVideo();
        var declare = SteamXBox.Tools.Serveurs.Ressources.CoutVideoMo();
    ...
    private void Peupler()
    {
        ...
        {
            0 => "Rien ne tourne au nom de SteamXBox.",
        ...
        if (inaccessibles > 0)
        {
            Resume.Text += $"  {inaccessibles} processus n'ont pas pu être examinés — "
                + "lancez SteamXBox en administrateur pour les voir.";
        }

        Section("SteamXBox", Famille.Natif, evenements,
        ...
        Section("Dépendances", Famille.Dependance, evenements,
            "Des programmes extérieurs. Ils peuvent avoir été lancés par autre chose que SteamXBox.");
    }
