...
public partial class DebugViewModel : ObservableObject
{
    ...
    private void RefreshCoreStatus()
        => UpdateCoreStatus(Process.GetProcessesByName("SteamXBox.Core").Length > 0);
    ...
    /// every game reads for player one — leaving SteamXBox's virtual pad in a slot nobody read.
    ///
    /// SteamXBox cannot reassign those slots; Windows does, in connection order. Saying so turns an
    ...
    private static string BuildWarning(ControllerSurvey survey)
    {
        ...
        // nothing to anyone, and no amount of SteamXBox configuration changes that.
        ...
        {
            return Strings.Current["Une autre manette occupe l'emplacement XInput 0, celui que les jeux lisent pour le joueur 1. La manette virtuelle de SteamXBox se retrouve après elle et n'est pas lue. Éteignez les autres manettes puis relancez le jeu."];
        }
    ...
    /// </summary>
    /// <remarks>
    /// The version is printed rather than checked. SteamXBox works with whatever version of these
