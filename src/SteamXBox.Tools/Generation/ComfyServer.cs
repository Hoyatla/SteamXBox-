...
public static class ComfyServer
{
    ...
    private static string? Lancer(Action<string>? journal, ref string place)
    {
        ...
        place = SteamXBox.Tools.Serveurs.Ressources.FairePlace(
            SteamXBox.Tools.Serveurs.Poids.Lourd, Ressource, journal);

        var lance = SteamXBox.Tools.Serveurs.ServeurLocal.Demarrer(
            Interpreteur,
            [
                "-u", Script,
            ...
            new SteamXBox.Tools.Serveurs.ServeurLocal.Identite(
                Ressource,
                "Générateur multimédia",
                "comfyui",
                SteamXBox.Tools.Serveurs.Poids.Lourd,

                CoutVideoMo: Disponible(),
                SteamXBox.Tools.Serveurs.ALaFermeture.Tuer));
    ...
    {
        // Passage par l'explorateur, et c'est la seule voie fiable ici : SteamXBox demande
