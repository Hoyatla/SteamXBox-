...
public static class Program
{
    ...
    {
        const string CoreProcess = "SenSÉ.Core";
    ...
    /// </summary>
    /// <remarks>
    /// Two executables are published from this one project: <c>SenSÉPads.Osk.exe</c> for a Steam
    /// Controller, which types on its trackpads, and <c>SenSÉSticks.Osk.exe</c> for a pad that
    ...
    ///
    /// Null when the name says nothing — the plain <c>SenSÉ.Osk.exe</c> — and the style then
    ...
    /// Focus the application under test, wait a few seconds, then read SenSÉ-osk-debug.log.
    /// </remarks>
    private static void ProbeCaret()
    {
        ...
        for (var i = 0; i < 12; i++)
        {
            ...
            try
            {
                ...
                var placement = SenSÉ.Core.Osk.OverlayPlacement.Place(
    ...
    private static string Describe(SenSÉ.Core.Osk.ScreenRect rect)
    ...
    private static void Run()
    {
        ...
        // session that ended days earlier made the keyboard appear on screen the instant SenSÉ
