...
/// <summary>
/// Tracks whether Steam is running and derives controller ownership from it, so SenSÉ hands
/// the controller over when Steam starts and takes it back when Steam closes — including when the
/// user closes Steam from Steam's own UI rather than through a SenSÉ shortcut.
/// </summary>
public sealed class SteamPresenceWatcher
{
    ...
    public ControllerOwner Owner { get; private set; } = ControllerOwner.SenSÉ;
    ...
    /// asks SenSÉ to launch Steam: waiting for the next poll would keep SenSÉ writing to
    ...
    /// <summary>
    /// Takes ownership back immediately, skipping the grace period. Used after SenSÉ itself
