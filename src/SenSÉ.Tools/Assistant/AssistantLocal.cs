...
public sealed class AssistantLocal
{
    ...
    private const string ConsigneFixe =
        ...
        + "L'ATELIER est l'établi de création, partagé entre l'utilisateur et toi. On y compose des "
        + "graphes de nœuds, dans deux espaces : CODAGE — écrire, exécuter, appeler un modèle — et "
        + "MULTIMEDIA — image, vidéo, montage. L'utilisateur y travaille à la main, et tu y "
        + "travailles par les verbes atelier_*. C'est le même établi : ce que tu composes, il le "
        + "voit et peut le corriger ; ce qu'il a posé, tu peux le reprendre.\n"
        + "Tu n'as pas à retenir ce qu'il contient. Il se décrit lui-même — atelier_aide pour tout "
        + "découvrir, atelier_catalogue_types pour un espace, atelier_type pour un nœud précis. "
        + "DEMANDE-LUI plutôt que de supposer un nom de nœud : un graphe bâti sur un type inventé "
        + "échoue à l'exécution, plusieurs minutes plus tard.\n"
        + "Emploie-le quand le travail a une FORME qui se garde et se rejoue — une chaîne d'étapes, "
        + "une production à refaire sur d'autres entrées. Pour un geste unique, fais-le directement "
        + "avec tes outils : composer un graphe pour une seule image coûterait plus que l'image.\n\n"
