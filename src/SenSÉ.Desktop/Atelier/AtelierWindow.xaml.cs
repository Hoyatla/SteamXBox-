...
namespace SenSÉ.Desktop.Atelier;
...
public partial class AtelierWindow : Window
{
    ...
    private void Prechauffer(IReadOnlyList<PluginManifest> outils)
    {
        ...
        if (!utile || !SenSÉ.Tools.Generation.ComfyServer.Installe)
        {
            return;
        }

        if (SenSÉ.Tools.Generation.ComfyServer.Repond())
        {
            return;
        }
        ...
        Task.Run(() => SenSÉ.Tools.Generation.ComfyServer.Preparer(Avancement));
    }
    ...
    ///
    /// <para>
    /// Par l'explorateur et non par une liste maison : SenSÉ demande les droits administrateur,
