...
public static class SearchIndexBuilder
{
    ...
    /// <summary>
    /// Ce que SenSÉ héberge : les programmes accueillis et leurs dossiers.
    /// </summary>
    /// <remarks>
    /// <b>Pourquoi d'abord.</b> Chercher depuis SenSÉ, c'est chercher d'abord dans SenSÉ. Un
    ...
    private static void OutilsDuProduit(List<SearchItem> items, Action<string>? log)
    {
        ...
        try
        {
            foreach (var programme in SenSÉ.Plugins.Reconnaissance.Lire(outils))
            {
                Add(items, programme.Dossier, SearchItemKind.Folder, IndexPlan.OutilsPriority);
                vus++;

                if (programme is { Forme: SenSÉ.Plugins.FormeProgramme.Application, Executable.Length: > 0 })
                {
