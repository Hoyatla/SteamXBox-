...
public partial class ToolsView : UserControl
{
    ...
    private void Refresh()
    {
        ...
        Status.Text = $"{rows.Length} outil(s). Les changements prennent effet au prochain démarrage "
            + "de l'environnement. Les outils livrés avec SenSÉ ne s'effacent pas d'ici : "
    ...
    private static string Pesee()
    {
        try
        {
            var fichiers = SenSÉ.Tools.Generation.PaquetGenerateur.Contenu(AppContext.BaseDirectory);
    ...
    private void CompresserClic(object sender, RoutedEventArgs e)
    {
        ...
        Travailler(
            "Empaquetage",
            arret => SenSÉ.Tools.Generation.PaquetGenerateur.Compresser(
    ...
    private void DecompresserClic(object sender, RoutedEventArgs e)
    {
        ...
        Travailler(
            "Déballage",
            arret => SenSÉ.Tools.Generation.PaquetGenerateur.Decompresser(
                AppContext.BaseDirectory, boite.FileName, Avancer, arret),
    ...
    private void AccueillirClic(object sender, RoutedEventArgs e)
    {
        ...
        Travailler(
            "Accueil",
            arret => Raconter(
                SenSÉ.Plugins.AccueilOutil.Installer(boite.FileName, dossier, null, null, PluginTools.Folder, arret)),
            "",
            AccueilEtat,
            () => Designer(dossier));
    }
    ...
    {
        if (SenSÉ.Plugins.Reconnaissance.Regarder(dossier) is not
            { Forme: SenSÉ.Plugins.FormeProgramme.PorteInconnue } programme)
        {
            return;
        }

        var portes = SenSÉ.Plugins.Reconnaissance.Portes(dossier);
        ...
        AccueilEtat.Text += Environment.NewLine
            + SenSÉ.Plugins.AccueilOutil.Declarer(programme, Root, boite.FileName);
    ...
    private static string Raconter(SenSÉ.Plugins.AccueilRapport rapport)
    {
    ...
    /// </remarks>
    private void Avancer(SenSÉ.Tools.Generation.AvanceePaquet ou)
    {
        if (ou.Total <= 0)
        {
            return;
    ...
    /// <summary>
    /// Says what would stay on the machine after SenSÉ is removed.
    /// </summary>
    /// <remarks>
    ...
    /// the product</i>, and the snapshot of what Windows had before lives in SenSÉ's own state
    ...
    /// values differ from the snapshot — the machine is wearing SenSÉ's cursors and the only
    ...
    private static string WindowsLeftBehind()
        => new SenSÉ.Shell.Theming.Windows.WindowsStateBackup().Exists
            ? "\n\nAttention : les curseurs de Windows ont été remplacés par SenSÉ. Ils le "
    ...
    private void Switch_Click(object sender, RoutedEventArgs e)
    {
        ...
        // Refused rather than merely absent from the list. Switching off what SenSÉ is made of
        // would remove the tile that opens the screen it would be switched back on from.
        if (ToolRegistry.IsSystem(id))
        {
            Status.Text = $"« {id} » fait partie de SenSÉ et ne se désactive pas.";
            Refresh();
            return;
        }
    ...
    private void Eject_Click(object sender, RoutedEventArgs e)
    {
        ...
        // rule is that SenSÉ does not delete what it shipped, and a rule enforced only by what
        ...
        {
            Status.Text = $"« {id} » est livré avec SenSÉ : il peut être désactivé ou archivé, "
        ...
        if (MessageBox.Show(question, "SenSÉ", MessageBoxButton.OKCancel, MessageBoxImage.Warning)
            != MessageBoxResult.OK)
        {
            return;
        }
    ...
    /// <param name="Protected">Shipped with SenSÉ: reversible operations only.</param>
    private sealed record Row(PluginEntry Entry, bool Archived, bool Compiled, bool Protected)
    {
        ...
        /// <summary>
        /// The same wording for everything SenSÉ ships, whether compiled in or loaded.
        /// </summary>
        /// <remarks>
