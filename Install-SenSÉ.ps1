...
param(
    ...
    # du 24 aout vers C:\Program Files\SenSÉ l'a rendu faux, et c'etait la seule reference
...
function Setup-SenSÉ {
    if ($NoSenSÉSetup) { Write-Log "Setup SenSÉ ignoré" -Color Yellow; return }
    
    Write-Log "=== Configuration SenSÉ Portable ===" -Color Cyan
    ...
    $required = @("SenSÉ.exe", "SenSÉ.Core.exe", "SenSÉ-Resident.cmd")
    ...
    Write-Log "Tous les fichiers SenSÉ présents" -Color Green
    
    # Créer raccourci Démarrage si demandé
    $startupDir = "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup"
    $shortcut = "$startupDir\SenSÉ.lnk"
    if (-not (Test-Path $shortcut)) {
        $ws = New-Object -ComObject WScript.Shell
        $sc = $ws.CreateShortcut($shortcut)
        $sc.TargetPath = Join-Path $InstallDir "SenSÉ.exe"
        $sc.Arguments = "xbox-run --restart --switch-button steam-or-quick-access"
        $sc.WorkingDirectory = $InstallDir
        $sc.Description = "SenSÉ - Manette Xbox pour Steam (résident)"
        $sc.Save()
    ...
    Write-Log "SenSÉ Portable configuré dans : $InstallDir" -Color Green
    return $true
}
...
function Show-Summary {
    ...
    Write-Bilan "SenSÉ" $global:SenSÉOK "ÉCHEC/IGNORÉ" 'Red'
    ...
    Write-Log ""
    Write-Log "Pour lancer SenSÉ :"
    Write-Log "  $InstallDir\SenSÉ.exe xbox-run --restart --switch-button steam-or-quick-access"
    Write-Log ""
    Write-Log "Pour arrêter :"
    Write-Log "  $InstallDir\SenSÉ.exe stop"
}

# ===== MAIN =====
Write-Log "=== INSTALLATEUR SenSÉ PORTABLE ===" -Color Cyan
