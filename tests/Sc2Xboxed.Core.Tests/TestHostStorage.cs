...
/// ceci, lancer la serie laissait onze identifiants <c>test-steamxbox-…</c> et un <c>essai</c> dans
/// <c>%LOCALAPPDATA%\SteamXBox\plugins-choices.json</c> — l'installation de la personne qui lance
...
internal static class TestHostStorage
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "steamxbox-tests-storage");
