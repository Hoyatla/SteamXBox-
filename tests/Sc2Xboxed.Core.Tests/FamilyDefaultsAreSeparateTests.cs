...
/// <c>Sc2XboxedProfileSettings.DefaultFor</c> — et les deux mappers d'une meme manette recevaient la
...
public class FamilyDefaultsAreSeparateTests
{
    ...
    public void ThePs5DefaultsComeFromThePs5File()
        => Assert.Equal(
            ...
            Sc2XboxedProfileSettings.DefaultFor(ControllerKind.DualSense));

    [Fact]
    public void TheXboxDefaultsComeFromTheXboxFile()
        => Assert.Equal(
            XboxControllerDefaults.Settings,
            Sc2XboxedProfileSettings.DefaultFor(ControllerKind.XInput));
    ...
    {
        var steam = Sc2XboxedProfileSettings.DefaultFor(ControllerKind.SteamController);
