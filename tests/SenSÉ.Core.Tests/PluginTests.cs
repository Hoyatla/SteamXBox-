...
public class CrsSchemeParserTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"SenSÉ-{Guid.NewGuid()}.crs");
        try
        {
...
public class PluginManifestTests
{
    ...
    {
        var scan = PluginCatalog.Scan(Path.Combine(Path.GetTempPath(), "SenSÉ-absent-" + Guid.NewGuid()));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "SenSÉ-plugins-" + Guid.NewGuid());
        try
        {
